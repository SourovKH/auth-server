...
type GoogleService interface {
	HandleGoogleCallback(c *gin.Context) (user auth_models.User, accessToken string, refreshToken string, err error)
}
...
func (service *googleService) HandleGoogleCallback(c *gin.Context) (user auth_models.User, accessToken string, refreshToken string, err error) {
	// 1. Verify state (skipped for brevity, but essential for production)
	
	// 2. Exchange code for token
	code := c.Query("code")
	token, err := auth_utils.GoogleOAuthConfig.Exchange(context.Background(), code)
	if err != nil {
