...
import (
	...
	google_service "lem-be/auth/services"
...
func NewGoogleHandler(googleService google_service.GoogleService) GoogleHandler {
...
func (h *googleHandler) HandleGoogleLogin(c *gin.Context) {
	...
	state := "random-state"
	url := auth_utils.GoogleOAuthConfig.AuthCodeURL(state)
